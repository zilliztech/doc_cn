...
在 MegaWise 中有三种隐式类型常量：字符串、位串和数字。你可以为常量指定显示类型，从而增加显示精确度和处理效率。
...
字节序列必须由字符集编码中合法的字符组成，特别是在使用八进制或十六进制转义时。如果编码为 UTF-8，那么应该使用 Unicode 转义或替代的 Unicode 转义语法。你也可以手动写出 UTF-8 编码字节，但会非常麻烦。
...
SELECT (5 !) - 6;
```
下表展示了 MegaWise 中操作符的优先级（最高到最低）和结合性。
