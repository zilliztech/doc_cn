...
在数图中，您可以通过各种计量方式和格式来显示数值。下图使用纽约出租车数据集显示了纽约出租车小费的平均值。
...
|`显示范围`     | 开启时，图表底部显示折线图的整体，使用鼠标拖拽任意一个区域就可以使上方图表放大显示该区域。 <br><br>  ![显示范围](../assets/line_chart_ch_range.png)    |
|`颜色数据映射`   |    自定义折线颜色。          |
...
在热力图中，您可以根据区域的颜色分析数据在不同维度区间的变化。下图中，X 轴和 Y 轴分别代表日期和时间，每个区域的颜色由特定日期和时间的小费平均占比决定。颜色越接近最大值的颜色，则小费平均占比越高。
