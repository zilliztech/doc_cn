...
- 局部放大显示：开启`显示范围`，图表底部显示折线图的整体，使用鼠标拖拽任意一个区域就可以使上方图表放大显示该区域。

  ![显示范围](../assets/line_chart_ch_range.png)

- 自动播放：任意选取一个区域，点击三角形的播放按钮，即可实现折线图自动播放。自动播放等价于一个移动的过滤条件，可以使仪表盘上的其他相关图表产生实时变化。
...
在折线图中用鼠标拖拽选择任意区域即可建立过滤条件。点击折线图左上方的删除过滤条件按钮即可删除过滤条件。
